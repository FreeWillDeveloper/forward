...
## 1. 핵심 문제: 데이터가 바뀌면, 인터페이스는 어떻게 접근법
...
### 1.2 JavaScript 변수가 바뀌었는데, 왜 인터페이스는 자동으로 바뀌지 않을까 동기
...
### 1.3 프레임워크는 무엇을 하는가 두 단계로 자동 연결 구축 개요
...
## 3. 반응형 시스템: 프레임워크는 데이터가 변경된 것을 어떻게 알까 접근법
...
## 5. DOM 조작의 비용: 왜 프레임워크는 이렇게 큰 노력을 기울일까 동기
...
## 6. 런타임 vs 컴파일 타임: 프레임워크 설계의 핵심 트레이드오프 비교
### 6.1 두 단계
