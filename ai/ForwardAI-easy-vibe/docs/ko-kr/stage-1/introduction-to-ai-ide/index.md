...
::: tip 💡 가장 중요한 학습 원칙: 궁금하면 먼저 AI에게 묻기
이제부터 AI를 **첫 번째 도움 요청 대상**으로 삼으세요. 개념을 이해하지 못했거나, 도구를 설치할 수 없거나, 명령어를 모르거나, 프로그램에서 오류가 나거나, 다음에 무엇을 해야 할지 모를 때도 **먼저 AI에게 묻고 직접 해 봅니다**.

예전이라면 선생님, 친구, 단체 채팅방에 물었을 질문을 먼저 AI에게 해도 됩니다. 답이 빠르고, 계속해서 물어볼 수 있으며, 오류 메시지와 프로젝트 상황을 함께 보고 다음 단계를 제안할 수 있습니다. 오래 막혀 있은 뒤에야 AI를 떠올리지 말고, ‘코드를 써 달라’고 할 때만 사용하지도 마세요.

**AI 네이티브 학습은 AI로 과제를 끝내는 데 그치지 않습니다. 문제가 생기면 묻고, 답을 직접 시험하고, 새 문제가 생기면 다시 묻는 학습 전 과정에 AI를 참여시키는 것입니다.**
:::

### AI 네이티브 학습 순환

AI를 활용한 능동 학습은 과제를 던져 놓고 결과를 기다리는 일이 아닙니다. 답을 찾는 시간을 줄이고 그만큼 **직접 해 보기, 확인하기, 이해하기**에 더 많은 시간을 쓰는 방식입니다. 다음 순환은 이 과정의 모든 장에서 사용할 수 있습니다.

```mermaid
flowchart LR
  A["① 문제를 만남<br/>목표와 현상을 설명"] --> B["② 즉시 AI에게 질문<br/>맥락도 함께 제공"]
  B --> C["③ 직접 실행<br/>제안을 따라 시도"]
  C --> D{"예상한 결과인가?"}
  D -- "아직 아님" --> E["④ 결과나 전체 오류를<br/>다시 AI에게 전달"]
  E --> B
  D -- "맞음" --> F["⑤ 자신의 말로<br/>왜 됐는지 설명"]
  F --> G["새 문제에도 적용할<br/>능력으로 만들기"]
```

### 자주 생기는 문제는 이렇게 바로 물어보세요

| 상황 | AI에게 할 말 |
| --- | --- |
| **개념을 이해하지 못함** | “프로그래밍을 처음 배웁니다. API가 무엇인지 일상적인 비유로 설명하고, 가장 작은 예제를 보여 주세요.” |
| **환경을 설치하지 못함** | “macOS에서 이 프로젝트를 실행하고 싶습니다. 필요한 환경을 먼저 확인한 뒤, 한 번에 한 단계만 안내하고 그 단계의 목적도 설명해 주세요.” |
| **명령 실행 중 오류 발생** | “실행한 명령과 전체 오류입니다. 가장 가능성 높은 원인을 먼저 판단하고 최소한의 수정 방법을 알려 주세요. 정보가 부족하면 무엇을 더 보내야 하는지 말해 주세요.” |
| **다음 할 일을 모름** | “스네이크 게임을 만드는 것이 목표이고 화면은 표시됩니다. 다음으로 할 가장 작은 작업과 완료 기준을 알려 주세요.” |
| **AI가 작성한 코드를 이해하지 못함** | “아직 코드를 더 바꾸지 마세요. 방금 수정한 내용을 모듈별로 나누어 각 부분이 어떤 문제를 해결하는지 설명해 주세요.” |
| **여러 방법 중 고르지 못함** | “초보자이고 실행 가능한 버전을 빨리 만들고 싶다는 조건으로 A와 B를 비교하고 하나를 분명히 추천해 주세요.” |
| **결과가 예상과 다름** | “원한 화면은 …이고 현재 화면은 …입니다. 차이를 비교한 뒤 가장 눈에 띄는 한 곳만 먼저 고쳐 주세요.” |

::: info 어디서나 쓸 수 있는 질문 틀
**목표는 …입니다. 현재 …까지 했습니다. …을 시도했습니다. 실제 결과나 전체 오류는 …입니다. 원인을 먼저 설명하고, 다음으로 할 가장 작은 작업을 알려 주세요.**

정보가 구체적일수록 AI가 상황에 맞는 답을 내놓기 쉽습니다. 첫 답으로 해결되지 않아도 정상입니다. 새 결과를 다시 보내는 과정 자체가 AI 네이티브 학습입니다.
:::
...
<StageAssignmentCard title="AI IDE로 나만의 게임 만들기">
