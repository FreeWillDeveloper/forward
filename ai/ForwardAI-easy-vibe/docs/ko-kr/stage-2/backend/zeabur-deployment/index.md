...
이 튜토리얼에서는 **서버를 구매할 필요도, DevOps를 배울 필요도 없는 가장 쉬운 방법으로 웹사이트를 공개**하는 과정을 알아봅니다. GitHub 저장소를 연결하고 버튼 몇 개만 클릭하면 웹사이트가 바로 공개됩니다. **텐센트 클라우드 CloudBase**, **Vercel**, **Netlify**, **Zeabur** 네 가지 인기 플랫폼을 다룹니다.
...
# PaaS 플랫폼을 쓰는 이유: 직접 서버를 세우지 않아도 되는 이유

"결국 모든 것이 '서버'에서 실행되는 것이라면, 차라리 서버를 직접 구매해서 배포하면 안 될까?"라고 궁금할 수 있습니다. 답은 간단합니다. **플랫폼이 모든 번거로운 부분을 대신 처리해 주기 때문입니다.**
