...
魔搭社群由達摩院聯合 CCF 開源發展委員會發起，除了開源模型和資料集，也提供展示應用的 **創空間（Studio）**。社群曾舉辦[核心開發者共創會](https://community.modelscope.cn/683562c6870cef7360622f7f.html)，讓不同城市的開發者交流開源專案。對我們來說，創空間最實用的一點是：不用先成為維運專家，也能為作品準備一個可以分享的地址。
...
其中最重要的是 SDK 類型必須選 **Static**。目前表單會提示 Static 適合展示已有 HTML 頁面，並提供 Gradio、Streamlit、Static 和 Docker 等選項。

![魔搭社群建立創空間頁面，在 SDK 區域選擇 Static](../../../zh-cn/stage-1/appendix-modelscope-static-site/images/modelscope-static-site/03-select-static.jpg)
...
儲存檔案後，平台通常會自動部署。如果沒有，就在創空間頁面點選部署、重新啟動或重新執行。等待狀態變為「執行中」，再開啟類似下面的地址：
