---
title: '手を動かしてプロトタイプを作る - 業務分析からマルチページプロダクトプロトタイプの実装まで'
...
::: info 💡 ツールについて
AI IDE の画面、利用できるモデル、利用枠は継続的に変わります。現在安定して使えるツールを選べば十分です。本章で扱う要件の伝え方、生成、確認、修正の流れは、特定のブランドに依存しません。コマンドラインのコーディングツールが必要な場合は、[現代の AI CLI ツール](../../stage-2/backend/modern-cli/)を参照してください。
:::
