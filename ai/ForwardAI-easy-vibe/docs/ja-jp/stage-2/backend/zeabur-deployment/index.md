...
このチュートリアルでは、サイトを公開する**最も簡単な方法**を紹介します——サーバーを購入する必要も、DevOps を学ぶ必要もありません。GitHub リポジトリを接続してボタンを数回クリックするだけで、サイトが公開されます。ここでは **Tencent Cloud CloudBase**、**Vercel**、**Netlify**、**Zeabur** という4つの人気プラットフォームを紹介します。

# なぜ PaaS プラットフォームを使うのか？自分でサーバーを立てる代わりに

「結局すべて『サーバーの上』で動くのなら、なぜ自分でサーバーを買ってデプロイしないのか？」と思うかもしれません。答えは：**プラットフォームが面倒な部分をすべて代わりに処理してくれる**からです。
