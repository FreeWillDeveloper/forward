...
如果“链、合约、Token”这些词对你还很陌生，也没有关系。我们先把它们讲明白，再开始操作。这一篇不连接真实钱包，也不花真钱，只在电脑里使用一套随时可以重置的练习环境。
...
但我们这次不会直接使用以太坊主网。主网会接触真实钱包、真实资产和手续费，不适合第一次练习。我们会在自己的电脑里启动一条 **本地模拟链**：它只有你自己在使用，账户和余额都是假的，关掉以后还可以全部重置。
...
我们可以在程序里写下规则：只有管理员能发行证书；每张证书都有不同编号；任何人都能查询证书现在属于谁。程序被放到链上以后，用户调用它，链就按照这些规则处理结果。
...
区块链原生币和 Token 不是一回事。以太坊里的 ETH 是网络原生资产，常用来支付手续费；我们这篇创建的 Token 则由自己的合约管理，只代表 Vibe Certificate，不值真钱。
...
![Ticketmaster 官方展示的 NFT 数字藏品界面](images/nft-real-ticketmaster.png)
...
![POAP 官方展示的 WNBA 活动参与徽章](images/nft-real-poap-wnba.webp)
...
我们做一个 **Vibe Certificate**，可以把它理解成一张最小数字证书：
...
![本地模拟链与编译环境的实际运行结果](images/nft-local-environment.jpg)
...
![合约编译、部署和第一次铸造的实际结果](images/nft-local-deploy-mint.jpg)
...
![普通账户失败和管理员再次铸造的实际结果](images/nft-local-permission-test.jpg)

这里的失败正是我们需要的结果。权限测试如果只验证管理员成功，没有验证普通账户失败，就只完成了一半。

## 7. 从头运行一次

最后重新启动本地实验，从编译一直跑到第二次铸造。下面是这次完整运行结果：

![Vibe Certificate 本地编译、部署、铸造与权限验证结果](images/nft-local-run-result.jpg)
...
这次跑通的不是一张 NFT 图片，而是一条完整的最小链路：

**本地链 → 合约部署 → 管理员铸造 → 查询所有权 → 普通账户被拒绝 → metadata 地址。**

它仍然只是合约实验，不是可以直接运营的平台。Ticketmaster 和 POAP 这类真实产品还需要领取页面、用户账户、发行后台、内容存储、密钥管理、监控、客服和合规。智能合约只负责其中一部分。
