...
你好，这一节我们把一个想法真正放进微信里，从空项目一直做到手机体验版。
...
![Uber 微信小程序中的真实叫车页面](images/wechat-uber-mini-program.png)
...
这篇会用贪吃蛇练习页面、交互、状态和发布流程。它体量小，操作结果容易检查，很适合第一次跟着做。

不过，正式发布时要注意：普通小程序和微信小游戏有不同的产品入口、类目和审核要求。我们这里先用普通 uni-app 小程序项目完成开发练习；如果你准备长期运营游戏、接入广告或排行榜，应按微信公众平台当前的小游戏规则重新确认账号和类目，不要把游戏随便填成教育、工具或其他不相符的类别。
...
这篇使用 HBuilderX 创建 uni-app 项目，再让 Trae 修改当前项目。每次修改后，都回到微信开发者工具查看真实结果。你不需要先学完全部语法，但要看得懂页面有没有运行、控制有没有失效，以及错误出现在什么位置。

所以这篇不会先讲一大堆语法。我们从空项目开始，做出贪吃蛇，再换成摇杆控制；每改一步，就去模拟器和手机上试一次，最后把能玩的版本上传成体验版。
...
![微信小程序操作截图](images/image4.png)![微信小程序操作截图](images/image5.png)
...
![微信小程序操作截图](images/image6.png)

接着从 [HBuilderX 官网](https://www.dcloud.io/hbuilderx.html) 下载对应版本。安装后能进入欢迎页即可，暂时不用研究其他功能。

![微信小程序操作截图](images/image7.png)

到这里应该已经有小程序账号、AppID、Trae、HBuilderX 和微信开发者工具。下面开始创建项目。

### 2.5 创建基础项目

1. 在 HBuilderX 中点击“新建项目”。

![微信小程序操作截图](images/image8.png)

2. 选择 uni-app 默认模板，填写项目名和保存位置，然后点击右下角“创建”。

![微信小程序操作截图](images/image9.png)

3. 等待 HBuilderX 创建项目。

![微信小程序操作截图](images/image10.png)

4. 在 Trae 中打开刚才创建的项目文件夹。左侧能看到项目文件，说明准备完成。

![微信小程序操作截图](images/image11.png)
...
> 请把当前 uni-app 项目改成贪吃蛇小程序。先做首页和游戏页，游戏要能开始、移动、吃食物、计分和重新开始。直接修改当前项目，完成后告诉我怎样运行。
...
![微信小程序操作截图](images/image12.png)![微信小程序操作截图](images/image13.png)
...
![微信小程序操作截图](images/image21.png)

### 3.6 最终成品

最后得到首页、游戏页和结束状态。蛇能移动、吃食物和计分，摇杆松开后仍会沿最后方向继续移动。

![微信小程序操作截图](images/image22.png)![微信小程序操作截图](images/image23.png)![微信小程序操作截图](images/image24.png)
...
![微信小程序操作截图](images/image27.png)

![微信小程序操作截图](images/image28.png)

![微信小程序操作截图](images/image29.png)![微信小程序操作截图](images/image30.png)
...
4. 在这一行右侧选择「设为体验版」。如果后台要求先补充主营类目，按实际功能选择后再回来操作。
   ...
   ![微信小程序操作截图](images/image31.png)

   ![微信小程序操作截图](images/image32.png)
