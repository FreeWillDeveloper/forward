...
这一篇就在原项目上增加后端。每次只接一项能力，然后同时去小程序页面和 CloudBase 控制台确认结果。
...
![Uber 微信小程序中的路线、订单和支付页面](../wechat-miniprogram/images/wechat-uber-mini-program.png)
...
> 请把当前项目改成一个客户服务小程序，名字叫 Northstar Service Hub。做会员首页、创建服务请求和我的工单三个页面，先用演示数据。不要新建项目，完成后告诉我怎么运行。
...
> 请简化首页，只保留会员状态、三个常用服务和最近一条工单。创建工单时，把问题信息和联系方式分开。页面不要出现技术词，完成后告诉我检查哪些地方。
...
> 请增加“获取当前用户”功能。用户身份必须由云函数自己获取，不要相信前端传来的身份。页面和日志不要显示完整 OpenID。完成后告诉我怎么测试。
...
> 请把“创建服务请求”接到云端。用户提交后，由云函数保存工单并返回工单编号。用户身份仍然由云函数获取。完成后告诉我部署哪个云函数，以及去哪里查看保存结果。
...
> 请给工单增加图片上传。图片放在云存储，数据库只保存文件标识。限制图片的数量、大小和格式，上传失败时不要清空表单。完成后告诉我怎么测试。
...
> 我提交工单后，页面一直显示“处理中”。开发者工具的错误是【粘贴错误】，云函数日志是【粘贴已脱敏的日志】。请找出原因，只做必要修改，然后告诉我怎么重新验证。
