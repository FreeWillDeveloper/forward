...
![BOMAG myCOCKPIT 真实 HMI 界面](images/qt-enterprise-bomag-mycockpit.png)

单独看界面还不容易理解它为什么属于工业软件。装进驾驶室以后就清楚了：中间的 Qt 显示屏只是整台机器的一部分，它要和控制器、摄像头、传感器及车辆总线一起工作。

![BOMAG 工程机械驾驶室中的 myCOCKPIT 显示终端](images/qt-enterprise-bomag-in-vehicle.jpg)
...
![Agile Robots 真实机器人示教终端](images/qt-enterprise-agile-robots-interface.jpg)
...
![Parker Pro Display 上的真实应用入口界面](images/qt-reference-parker-hmi.png)
...
![Blue Ctrl X-CONNECT 船舶自动化平台的真实界面](images/qt-reference-blue-ctrl.jpg)
...
![Precision Planting 20|20 农业设备实时监视器](images/qt-reference-precision-planting.webp)
...
![下位机读取现场输入、执行控制逻辑并驱动设备](images/qt-lower-computer-explained.svg)
...
![Qt 上位机把设备数据整理成状态、趋势、报警和操作入口](images/qt-upper-computer-explained.svg)
...
![传感器、下位机、通信协议和 Qt 上位机之间的关系](images/qt-upper-lower-architecture.svg)
...
好，概念先说到这里。下面我们就从一个能在桌面运行的设备运营客户端开始，亲手把这条链路跑通。它会显示压力、温度和振动，设备断线时给出提醒，出现异常时生成报警，并把报警记录保存在本机。
...
这一篇先用 PySide6，是因为不用一开始就配置 C++ 编译器和 CMake，比较容易把数据、页面、报警和打包完整做一遍。PySide6 是 Qt 官方的 Python 绑定，企业里的内部工具、测试程序、诊断软件和第一版原型也会使用它。
...
现在回头看，这一篇做的并不只是一张深色仪表盘。一个小型上位机最基本的链路已经连了起来：
...
到这里，你应该已经能看出 Qt 工业软件真正难的地方了。它不只是把数字画得漂亮，而是设备在线时看得懂，设备离线时不骗人，出现异常时留得下记录，换一台电脑以后仍然能够稳定启动。这才是一个上位机项目真正的起点。
