...
from mistral_ocr_pipeline.pipeline import (
...
# ---------------------------------------------------------------------------
# Mistral client — cached across reruns
# ---------------------------------------------------------------------------
...
def init_mistral_client():
    ...
    if not api_key:
        return None, "MISTRAL_API_KEY not set in environment"
    try:
...
def _render_figure_intelligence_tab(state: ScienceFlowState) -> None:
    ...
    shown = [
        ...
        for fi in state.figure_intelligences
        if fi.is_quantitative_chart and fi.passes_sanity_check
    ]
    discarded = len(state.figure_intelligences) - len(shown)
    if discarded:
        st.caption(
            f"Showing {len(shown)} of {len(state.figure_intelligences)} figures — "
            f"{discarded} discarded (not a data chart, or failed the image/caption "
            "sanity check)."
        )

    if not shown:
        st.info("No figures passed the chart-type / sanity checks.")
        return

    total = len(shown)
    for idx, fi in enumerate(shown):
...
with st.sidebar:
    ...
    client, client_err = init_mistral_client()
...
with col_header:
    ...
    powered_by_html = """
        <div style='display: flex; align-items: center; gap: 10px; margin-top: 5px;'>
            ...
            <img src="https://docs.mistral.ai/assets/logos/vibe.svg" width="40">
        </div>
    """
...
if analyze_button and uploaded_file is not None and client is not None:
    ...
    try:
        ...
        with st.status(
            ...
            st.write("Uploading and running Mistral OCR 4 with Document AI extraction…")
            try:
                from mistralai.client.errors import SDKError

                pipeline_results = run_science_pipeline(client, tmp_path)
                figures, page_count, quality_score = parse_pipeline_results(
                    pipeline_results
                )
                ...
                status_pipeline.update(label="Mistral OCR request failed", state="error")
                st.error(f"Mistral OCR error: {exc}")
                st.stop()
            except RuntimeError as exc:
                status_pipeline.update(label="Result parsing failed", state="error")
                st.error(str(exc))
                if pipeline_results is not None:
                    raw = getattr(pipeline_results.get("ocr"), "document_annotation", None)
                    if raw is not None:
                        with st.expander("Raw document_annotation (for debugging)"):
                            st.code(str(raw)[:4000], language="json")
                st.stop()
        ...
        if not figures:
            ...
            if pipeline_results is not None:
                raw = getattr(pipeline_results.get("ocr"), "document_annotation", None)
                if raw is not None:
                    with st.expander("Debug: raw document_annotation"):
                        st.code(str(raw)[:4000], language="json")
            st.stop()
        ...
        with st.status("Analyzing Figures...", expanded=True) as status_analyst:
            ...
            if flow.state.quality == "poor":
                ...
                q_str = f" · avg page confidence {round(q * 100)}%" if q is not None else ""
                label = (
